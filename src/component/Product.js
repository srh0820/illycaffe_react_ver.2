...
function Product() {
    ...
    const backgroundColors = ['#E65C3F', '#C19B74', '#944D5B', '#E28E36', "#C0653E"];
        ...
        <section className={pdscss.product}>
            {isSlide ? (
                ...
                <ul className={`${pdscss.productUl} d-flex justify-content-center mb-0`}>
                    {products.map((product, index) => {
                        const hoverBackgroundColor = backgroundColors[index % backgroundColors.length];
                        return (
                            <li className={pdscss.productBox} key={index}>
                                <div 
                                    className={`${pdscss.hoverBox} d-flex`} 
                                    onMouseEnter={(e) => e.currentTarget.style.backgroundColor = hoverBackgroundColor}
                                    onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#121013'}
                                >
